...
module lp_account::liquidity_pool {
    ...
    */
	...
	let liquidity_signer_cap = resource_account::retrieve_resource_account_cap(admin, @overmind);
        let liquidity_create_event = account::new_event_handle<CreateLiquidityPoolEvent>(admin);
        let supply_event = account::new_event_handle<SupplyLiquidityEvent>(admin);
        let remove_event = account::new_event_handle<RemoveLiquidityEvent>(admin);
        let swap_event = account::new_event_handle<SwapEvent>(admin);
        move_to(admin, State {
            signer_cap: liquidity_signer_cap,
            create_liquidity_pool_events: liquidity_create_event,
            supply_liquidity_events: supply_event,
            remove_liquidity_events: remove_event,
    ...
    */  
	...
	let resource = borrow_global_mut<State>(@lp_account);
        let signer_cap = account::create_signer_with_capability(&resource.signer_cap);
        let address_of_signer = std::signer::address_of(&signer_cap);
        let coinA_type = type_info::type_name<CoinA>();
        let coinB_type = type_info::type_name<CoinB>();
        let coin_name;
        let coin_symbol;        

        if (exists<LiquidityPool<CoinA, CoinB>>(address_of_signer)){
                abort ECodeForAllErrors //abort if the liquidity pool does not exist
        }
        else {

            //cannot use exists on coinA and CoinB since they don't have the key ability
            if ((coin::is_coin_initialized<CoinA>()) && (coin::is_coin_initialized<CoinB>())){
                if (comparator::is_smaller_than(&(comparator::compare(&coinA_type, &coinB_type)))){
                    //assemble lp coin name
                    coin_symbol = string::utf8(b"");
                    coin_name = string::utf8(b"");
                    ...
                    move_to(&signer_cap, LiquidityPool<CoinA, CoinB>{
                        coin_a_reserve: coin::zero(),
                        coin_b_reserve: coin::zero(),
                        lp_coin_mint_cap: mint_cap,
                        lp_coin_burn_cap: burn_cap,
                    });
                    //destroy the freeze cap 
                    coin::destroy_freeze_cap(freeze_cap);
                    coin::register<LPCoin<CoinA, CoinB>>(&signer_cap);
                    //dispatch event 
                    event::emit_event<CreateLiquidityPoolEvent>(&mut resource.create_liquidity_pool_events,
                    ...
                    abort ECodeForAllErrors //abort when the coins are not sorted
                }
            }else {
                abort ECodeForAllErrors // abort when the coins don't exist 
            }
        }
    ...
    */
	...
	let resource = borrow_global_mut<State>(@lp_account);
        ...
        let coin_a_value = (coin::value(&coin_a) as u128);
        let coin_b_value = (coin::value(&coin_b) as u128);
        let lp_coin: Coin<LPCoin<CoinA, CoinB>>;
        let minimum_amount= 1000;
        let lp_amount;
        
        if (exists<LiquidityPool<CoinA, CoinB>>(address_of_signer)){ //maybe we can use @lp_account instead
            
            let lp_pool = borrow_global_mut<LiquidityPool<CoinA, CoinB>>(address_of_signer);
            let smaller_than_compairison = (comparator::compare(&coin_a_type, &coin_b_type));
            std::debug::print(&std::string::utf8(b"this is the comparator"));
            std::debug::print(&smaller_than_compairison);
            let smaller_than = comparator::is_smaller_than(&smaller_than_compairison);
            

            assert!(smaller_than, ECodeForAllErrors); 
            // might need to change this to use the Result attribute of the comparator
            assert!(!(comparator::is_equal(&(comparator::compare(&coin_a_type, &coin_b_type)))), ECodeForAllErrors);
            


            assert!((coin_a_value > 0 && coin_b_value > 0), ECodeForAllErrors);
            
            let coin_a_reserve_value = coin::value(&lp_pool.coin_a_reserve);
            let coin_b_reserve_value = coin::value(&lp_pool.coin_b_reserve);

            
            

            if (coin_a_reserve_value == 0 && coin_b_reserve_value == 0){
                assert!((math128::sqrt(coin_a_value * coin_b_value)) > minimum_amount, ECodeForAllErrors);
                let minimum_coin = coin::mint((minimum_amount as u64), &lp_pool.lp_coin_mint_cap);

                coin::deposit(address_of_signer, minimum_coin);
                lp_amount = math128::sqrt(coin_a_value * coin_b_value);
                lp_coin = coin::mint(((lp_amount - minimum_amount) as u64), &lp_pool.lp_coin_mint_cap);

            }    
            else {
                let total_supply_option = coin::supply<LPCoin<CoinA, CoinB>>();
                let lp_coins_total_supply = option::extract(&mut total_supply_option);
                let a_mint_value = (coin_a_value * (lp_coins_total_supply * 1_000_000 / (coin_a_reserve_value as u128))) / 1_000_000;
                let b_mint_value = (coin_b_value * (lp_coins_total_supply * 1_000_000 / (coin_b_reserve_value as u128))) / 1_000_000;
                let mint = math128::min(a_mint_value, b_mint_value);
                lp_coin = coin::mint((mint as u64), &lp_pool.lp_coin_mint_cap);
                lp_amount = mint;
            };
        ...
        let state = borrow_global_mut<State>(address_of_signer);
        event::emit_event<SupplyLiquidityEvent>(
            ...
            SupplyLiquidityEvent {
                ...
                coin_a: coin::name<CoinA>(),
                coin_b: coin::name<CoinB>(),
                amount_a: (coin_a_value as u64),
                amount_b: (coin_b_value as u64),
    ...
    */
	...
	let lp_coins_value = coin::value(&lp_coins_to_redeem);
        let state = borrow_global_mut<State>(@lp_account);
        let signer_cap = account::create_signer_with_capability(&state.signer_cap);
        let lp = borrow_global_mut<LiquidityPool<CoinA, CoinB>>(std::signer::address_of(&signer_cap));
        let coin_a_reserve = coin::value(&lp.coin_a_reserve);
        let coin_b_reserve = coin::value(&lp.coin_b_reserve);
        let lp_coins_option = coin::supply<LPCoin<CoinA, CoinB>>();
        let total = option::extract(&mut lp_coins_option);
        let cpin_a_amount = (lp_coins_value as u128) * (((coin_a_reserve * 1_000_000 as u128) / total)) / 1_000_000;
        let coin_b_amount = (lp_coins_value as u128) * (((coin_b_reserve * 1_000_000 as u128) / total)) / 1_000_000;

        assert!((lp_coins_value > 0),
            abort ECodeForAllErrors);
        
        coin::burn(lp_coins_to_redeem, &lp.lp_coin_burn_cap); 

        let coin_a_out = coin::extract(&mut lp.coin_a_reserve, (cpin_a_amount as u64));
        let coin_b_out = coin::extract(&mut lp.coin_b_reserve, (coin_b_amount as u64));

        event::emit_event<RemoveLiquidityEvent>(&mut state.remove_liquidity_events, RemoveLiquidityEvent {
            coin_a: coin::name<CoinA>(),
            coin_b: coin::name<CoinB>(),
            lp_amount: lp_coins_value,
            amount_a: (cpin_a_amount as u64),
    ...
    */
	...
	let coin_a_type = type_info::type_name<CoinA>();
        ...
        if ((coin_a_in_value == 0 && coin_b_in_value > 0) || (coin_a_in_value > 0 && coin_b_in_value == 0) || (coin_a_in_value > 0 && coin_b_in_value > 0)) {

        assert!(exists<LiquidityPool<CoinA, CoinB>>(@lp_account), ECodeForAllErrors);
        let lp = borrow_global_mut<LiquidityPool<CoinA, CoinB>>(@lp_account);

        let coin_a_reserve_amount = coin::value(&lp.coin_a_reserve);
        let coin_b_reserve_amount = coin::value(&lp.coin_b_reserve);
        let k_return = coin_a_reserve_amount * coin_b_reserve_amount ;
        let k_initial = (coin_a_reserve_amount - amount_coin_a_out + coin_a_in_value) * (coin_b_reserve_amount - amount_coin_b_out + coin_b_in_value);

        assert!((comparator::is_smaller_than(&(comparator::compare(&coin_a_type, &coin_b_type))) == true) || !(comparator::is_equal(&(comparator::compare(&coin_a_type, &coin_b_type))) == false) || !(exists<LiquidityPool<CoinA, CoinB>>(@lp_account)),
            abort ECodeForAllErrors
        );
        assert!((k_initial >=  k_return),
            abort ECodeForAllErrors
        );
         assert!((!(amount_coin_a_out > coin_a_reserve_amount) || !(amount_coin_b_out > coin_b_reserve_amount)),
             ECodeForAllErrors
        );
        
        coin_b_out = coin::extract(&mut lp.coin_b_reserve, amount_coin_b_out);
        coin_a_out = coin::extract(&mut lp.coin_a_reserve, amount_coin_a_out);

        let state = borrow_global_mut<State>(@lp_account);
        coin::merge(&mut lp.coin_a_reserve, coin_a_in);
        coin::merge(&mut lp.coin_b_reserve, coin_b_in);
        event::emit_event<SwapEvent>(&mut state.swap_events, SwapEvent {
            coin_a: coin::name<CoinA>(),
            coin_b: coin::name<CoinB>(),            
            amount_coin_a_in: coin_a_in_value,
            amount_coin_a_out,
            amount_coin_b_in: coin_b_in_value,
            amount_coin_b_out,
            creation_timestamp_seconds: timestamp::now_seconds()
        });
